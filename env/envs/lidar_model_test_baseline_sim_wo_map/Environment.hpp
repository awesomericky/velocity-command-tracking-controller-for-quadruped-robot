...
namespace raisim
{
    ...
    public:
        ...
        : RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable)
        {
            ...
            anymal_box_ = world_->addBox(1.1, 0.55, 0.2, 0.1, "default", raisim::COLLISION(2), raisim::COLLISION(3));
                ...
                if (lidar_scan_depth_modified[discrete_idx[i] + 1] != 1. && 2 <= num_scans) {
                    ...
                    if (num_scans <= 5) {
                        Eigen::Vector3d obstacle_center_W =
                                (scans[discrete_idx[i] + 1]->getPosition() +
                                 scans[discrete_idx[i + 1]]->getPosition()) / 2;
                        ...
                        obstacle_spheres.push_back(
                                world_->addSphere(max_distance, 1, "default", raisim::COLLISION(3),
                                                  raisim::COLLISION(2)));
...
//        // Should set below (1.0 - discretize_margin) because if there are lidar_scan_values close to 1.0 (ex) 0.99),
//        // the odd scan positions (0, 0, 100) that we set for default will be used in computation.
//        double safety_margin = 0.3; // 3 [m] // 3 [m] (because ray_length is 10 [m])
//        double discretize_margin = 0.4 / ray_length;
//        for (int i=0; i<scanSize; i++) {
//            if (lidar_scan_depth[i] > safety_margin)
//                lidar_scan_depth_modified[i] = 1.;
...
//        /// set object (sphere or box) on the clustered place
//        for (int i = 0; i < discrete_idx.size() - 1; i++) {
//            if (lidar_scan_depth_modified[discrete_idx[i] + 1] != 1.) {
//                /// set sphere
//                if (discrete_idx[i + 1] - discrete_idx[i] <= 5) {
//                    Eigen::Vector3d obstacle_center_W =
...
//            final_starting_idx = 0;
//
//        if (lidar_scan_depth_modified[final_starting_idx] != 1.) {
//            /// set sphere
//            int num_scans = (scanSize - final_starting_idx) + discrete_idx[0];
//            if (num_scans <= 5) {
//                Eigen::Vector3d obstacle_center_W =
...
//        }
    ...
    /// Set object for collision checking
    std::vector<raisim::Sphere *> obstacle_spheres;
    std::vector<raisim::Box *> obstacle_boxes;
