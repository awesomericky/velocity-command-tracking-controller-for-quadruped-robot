...
for update in range(environment_model_iteration_number, cfg["environment"]["max_n_update"]):
    start = time.time()
    ...
    if update % cfg["environment"]["eval_every_n"] == 0:
        ...
        command_sampler_cvae_idx = np.where(env_command_sampler_idx == 3)[0]
        ...
        for step in range(n_steps):
            ...
            if new_command_time:
                ...
                sample_user_command_cvae_traj = w_cvae_sampler(torch.from_numpy(temp_state.astype(np.float32)).to(device),
                ...
                sample_user_command[command_sampler_cvae_idx, :] = sample_user_command_cvae_traj[0, command_sampler_cvae_idx, 0, :]
