...
class Lidar_environment_model(nn.Module):
    def __init__(self, lidar_encoding_config, command_encoding_config, recurrence_config, prediction_config, device):
        super(Lidar_environment_model, self).__init__()
        self.lidar_encoding_config = lidar_encoding_config
        ...
        assert self.lidar_encoding_config["activation"] in list(self.activation_map.keys()), "Unavailable activation."
    ...
    def set_module(self):
        self.lidar_encoder = MLP(self.lidar_encoding_config["shape"],
                                 self.activation_map[self.lidar_encoding_config["activation"]],
                                 self.lidar_encoding_config["input"],
                                 self.lidar_encoding_config["output"])
        ...
        # self.lidar_encoder.to(self.device)
        # self.command_encoder.to(self.device)
        # self.recurrence.to(self.device)
        # self.Pcol_prediction.to(self.device)
        # self.coordinate_prediction.to(self.device)

    def forward(self, state, command_traj, training=False):
        """
        ...
        encoded_state = self.lidar_encoder.architecture(state).unsqueeze(0)
